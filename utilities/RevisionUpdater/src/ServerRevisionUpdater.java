...
public class ServerRevisionUpdater {
	
	//Constants.
	public static final String VERSION_DECLARATION_PEDEGREE = "private static String sSoftwareVersion";
	public static final String VERSION_DECLARATION_TEMPLATE = "%s = \"Version 0.0.0.%s\";";
	...
	public ServerRevisionUpdater(File targetFile, File subversionEntriesFile) throws IOException {
		...
		switch(args.length) {
			case 2: {
				try {
					...
					ServerRevisionUpdater updater = new ServerRevisionUpdater(new File(args[0]), new File(args[1]));
					...
					int declarationPosition = updater.seek(ServerRevisionUpdater.VERSION_DECLARATION_PEDEGREE);
					
					//This program was added in revision 912.
					if(currentRevision >= 912) {
						...
						if(declarationPosition >= 0) {
							...
							System.out.println("Unable to find Constants version entry.");
						}
					} else {
			...
			default: {
				System.out.println("Usage: ConstantsRevisionUpdater [Location of Constants.java] [Location of any /.svn/entries]");
				break;
			}
		}
	}
